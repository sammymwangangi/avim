    ...
    <div class="row pb-2">
      ...
      <div class="col-8 col-xs-6 col-sm-8 col-md-8 col-lg-8">
        <div class="form-group">
          ...
          <input type="number" min="1" id="quantity" v-model="quantity" class="rounded-0 form-control"
                 value="1">
        </div>
        <div class="form-group">
          <label for="productQuantity" class="sr-only">Product Quantity</label>
          <select class="form-control rounded-0" id="productQuantity" @change="onChange($event)" v-model="price">
        ...
        <button class="btn btn-danger btn-sm rounded-0 float-right" type="button"
                @click="$bvModal.hide(`${product.url}-${displayId}`)"><fa icon="times"></fa>
        </button>
        </div>
      </div><!-- end image column-->
    </div>
